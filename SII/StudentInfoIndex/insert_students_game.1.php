        ...
        $(document).ready(function () {
            $('.add_field').click(function (e) {
                ...
                $('.input_area').append('<div style="margin-top: 2px">'+
                  ...
                  '<span>Game Name:</span>'+
                  '<select type="text" name="game_name[]"> <option>Select One</option><?php  $select="SELECT COUNT(`game_id`) AS `row` FROM `game_info`  ";  $q=mysqli_query($con_db,$select);  $fetch=mysqli_fetch_array($q);  $row1=$fetch['row'];  $se="SELECT `game_name` FROM `game_info`LIMIT $row1";  $qe=mysqli_query($con_db,$se); if(!$qe) { echo "okkk".mysqli_error($con_db); }  while ($r=mysqli_fetch_array($qe)) {  echo "<option>";  echo $r['game_name'];  echo "</option>"; }?>'+
                 '</select>'+
                  '<span>Score:</span>'+
...
<div class="wrap">
  ...
  <div class="containerr">
    ...
    <section id="contentt">
      ...
      <div class="inside">
       <h2> Insert student game Info</h2>

       <li><a href="course_info.php">Return student game Info</a></li>
       <br>
       <br>
        <form action="insert_students_game.2.php" method="post">
              ...
              <div>
                  ...
                  <select type="text" name="game_name[]">
                       <option>Select One</option>
                        <?php
                        $select="SELECT COUNT(`game_id`) AS `row` FROM `game_info`  ";
                        $q=mysqli_query($con_db,$select);
                        $fetch=mysqli_fetch_array($q);
                        $row1=$fetch['row'];
                        $se="SELECT `game_name`
                        FROM `game_info`LIMIT $row1";
                        $qe=mysqli_query($con_db,$se);
                        if(!$qe)
                        {

        ...
        $(document).ready(function () {
            $('.add_field').click(function (e) {
                ...
                $('.input_area').append('<div style="margin-top: 2px">'+
                  '<span>Student Id:</span><select type="text" name="id[]"><option>Select One</option><?php $select="SELECT count(`student_id`) AS `row` FROM `student_personal_info`";$q=mysqli_query($con_db,$select);  $fetch=mysqli_fetch_array($q);  $row=$fetch['row'];  $se="SELECT `student_id` FROM `student_personal_info` LIMIT $row";  $qe=mysqli_query($con_db,$se);  if(!$qe) {  echo "problem".mysqli_error($con_db); } while ($r=mysqli_fetch_array($qe)) { echo "<option>";  echo $r['student_id'];  echo "</option>";  }  ?>'+

                    
                ' </select>'+
                  '<span>Hall Name:</span>'+
                  '<select type="name" name="hallname[]"> <option>Select One</option> <?php $select="SELECT count(`hall_id`) as `row` from `hall_info`  ";$q=mysqli_query($con_db,$select);$fetch=mysqli_fetch_array($q);$row=$fetch['row'];$se="SELECT `hall_name` from `hall_info` limit $row";$qe=mysqli_query($con_db,$se);if(!$qe){echo "okkk".mysqli_error($con_db);}
                    while ($r=mysqli_fetch_array($qe)) {echo "<option>";echo $r['hall_name'];echo "</option>";$row=$row-1;}?>'+
         '</select>'+
     ...
     <div id="banner">
      ...
      <div class="inside">
         <h2><font color="#008b8b">Admin Area</font></h2>
           ...
           <a href="student_hall.php" class="btn btn-inverse"><font color="blue">Return  Student Hall Info</font></a>
            <br>  <br>

       <h6>Insert Students Hall Info</h6>
       

         <br>
        <form action="insert_student_hall.2.php" method="post">

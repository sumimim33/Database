        ...
        $(document).ready(function () {
            $('.add_field').click(function (e) {
                ...
                $('.input_area').append('<div style="margin-top: 2px">'+
                  ...
                  '<select type="text" name="contestname[]"><option>Select One</option> <?php $select="SELECT COUNT(`contest_id`) AS `row` FROM `contest_info`  "; $q=mysqli_query($con_db,$select); $fetch=mysqli_fetch_array($q);  $row1=$fetch['row'];  $se="SELECT `contest_name`  FROM `contest_info`LIMIT $row1";  $qe=mysqli_query($con_db,$se); if(!$qe)  {  echo "okkk".mysqli_error($con_db);  }  while ($r=mysqli_fetch_array($qe)) { echo "<option>";  echo $r['contest_name'];  echo "</option>";  }  ?>'+
                  '</select>'+
                  '<span>Position:</span>'+
    ...
    <div class="container">
      ...
      <nav>
          <ul>
                    ...
                    <li><a href="#" class="m4">Others</a></li>
    ...
    <section id="contentt">
     <div id="banner">
      ...
      <div class="inside">
       ...
       <li><a href="student_contest_info.php">Return Student Contest Info</a></li>
      <br>
      <br>

        <form action="insert_student_contest.2.php" method="post">
              ...
              <div>
                  ...
                  <span>Contest Name:</span>
                   <select type="text" name="contestname[]">
                       <option>Select One</option>
                        <?php
                        $select="SELECT COUNT(`contest_id`) AS `row`
                                 FROM `contest_info`  ";
                        //echo $select;
                        $q=mysqli_query($con_db,$select);
                        $fetch=mysqli_fetch_array($q);
                        $row1=$fetch['row'];
                        $se="SELECT `contest_name`
                        FROM `contest_info`LIMIT $row1";
                        //echo $se;
                        $qe=mysqli_query($con_db,$se);
                        if(!$qe)
                        {
